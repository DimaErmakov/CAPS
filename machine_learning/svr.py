...
# INPUT_CSV = 'music-hold-out.csv'
OUTPUT_CSV = 'ErmakovDimitry.csv'
...
# df["Release_Year_chat"] = pd.to_numeric(df["Release_Year_chat"], errors="coerce") #
df["Exact_Youtube_Publish_Date"] = pd.to_datetime(df["Exact_Youtube_Publish_Date"]).map(lambda x: x.toordinal())
df["Genre_chat"] = df["Genre_chat"].astype("category").cat.codes
# print (df["Exact_Youtube_Publish_Date"])
# print (df["Release_Year_chat"])
print(df)



#import from csv, split data into training, test, and prediction sets. 
#Training will be 80% of songs with known ratings
#test set will be 20% of known ratings withheld to score
#prediction set will be all songs with no known ratings
#mainly trust the numbers from youtube and not the chat generated attributes, especially for BPM data
x = []
# Use model selection techniques, change attributes/features, and try different kernel functions and margin hardness values (C) to get the best predictions you can
# try differnt C values (for loop), loop through one to all attributes
...
#predict and then score test set, then predict the ratings for the prediction set with the best model.
# print( classifier.predict([]))

# after done predicting, add predictions to the original csv. 
#output .csv file should look identical to music-hold-out.csv but with the missing entries filled in.
# only these coloumns should be exported Artist: Song,URL,x0,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24,x25,x26,x27,x28,x29,x30,x31,x32
