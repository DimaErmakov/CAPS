...
__attribute__((always_inline)) inline void multiply(double *__restrict C, const double *__restrict A, const double *__restrict B, unsigned int N)
{
  ...
  for (unsigned int i = 0; i < N; ++i)
    for (unsigned int k = 0; k < N; ++k)
    {
      double a = A[i * N + k];
      for (unsigned int j = 0; j < N; ++j)
        C[i * N + j] += a * B[k * N + j]; // B is now row wise
    }
}

void strassen(double *C, const double *A, const double *B, unsigned int N)
{
  if (N <= 64)
  {
    multiply(C, A, B, N);
